...
-- Add is_nida_verified column to citizen table if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name = 'citizen' AND column_name = 'is_nida_verified') THEN
    ALTER TABLE "citizen" 
    ADD COLUMN "is_nida_verified" boolean NOT NULL DEFAULT false;
  END IF;
END $$;

-- Add unique constraint to nida_number in citizen table if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
                 WHERE table_name = 'citizen' AND constraint_name = 'UQ_citizen_nida_number') THEN
    ALTER TABLE "citizen"
    ADD CONSTRAINT "UQ_citizen_nida_number" UNIQUE ("nida_number");
  END IF;
END $$; 